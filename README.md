...
## 🚀 新しいビジネスモデル：診断結果に基づく自動シナリオ分岐

このLPの最大の特徴は、**フロントエンド（Next.js）とプロラインを完全に分離した設計**にあります。これにより、診断結果に基づいて自動的に異なるLINEシナリオを実行できる、新しいビジネスモデルが実現できます。

### 💡 仕組みの概要

**診断の時点でAタイプ／Bタイプを決めて、その結果を`free1`に乗せておけば、プロライン側でその値を使って分岐できます。**

#### 具体的なイメージ

**Next.js側（フロントエンド）:**
- 診断で「あなたはAタイプです」→ `free1=A` を付けて友だち追加URLへ遷移
- 「あなたはBタイプです」→ `free1=B` を付けて遷移
- 診断内容を詳細に記載 → `free1=【診断結果】人数:2人...` を付けて遷移

**プロライン側（バックエンド）:**
- ユーザー情報1（`free1`）が「A」の人 → Aタイプ用シナリオに自動移動
- ユーザー情報1（`free1`）が「B」の人 → Bタイプ用シナリオに自動移動
- それぞれに「Aタイプ」「Bタイプ」タグも自動付与
- 診断内容が詳細な場合 → `[[free1]]`で診断内容を表示し、内容に応じて適切なシナリオに分岐

### 🎯 ビジネス価値

この仕組みにより、**「診断結果ごとに違うLINEステップ・案内」を自動で出し分けられる**ようになります。

#### 活用例

1. **顧客タイプ別の自動分岐**
   - 予算重視型 → コスパ重視のメッセージ・シナリオ
   - 贅沢重視型 → 高級ネタ推奨のメッセージ・シナリオ
   - 家族向け → 子ども向けメニュー推奨のシナリオ
...
3. **業種を超えた展開**
   - 寿司店だけでなく、あらゆるカスタマイズ可能なサービスに応用可能
   - 美容院、整体、カフェ、レストランなど
...
**プロライン側の設定:**
1. シナリオメッセージ内で`[[free1]]`を使用
2. ユーザー情報1（`free1`）の値に応じて条件分岐を設定
3. 各タイプごとに専用のシナリオを用意

### 💼 ビジネスモデルとしての可能性

この仕組みを活用することで、以下のような新しいビジネスモデルが可能になります：

1. **診断型LP制作サービス**
   - 顧客の診断結果に基づいて、自動的に最適なLINE対応を提供
   - 店舗側の手作業を削減しながら、顧客満足度を向上

2. **パーソナライゼーション自動化サービス**
   - 診断結果を元に、顧客ごとに最適化されたコミュニケーションを自動実行
   - マーケティングオートメーションの新たな形

3. **業種横断型プラットフォーム**
   - 診断フォームのテンプレートを業種ごとに提供
   - プロラインとの連携を標準化

### 📚 参考資料

- [プロライン公式マニュアル：外部サイトからの友だち追加パラメータ](https://autosns.co.jp/manual/start/external/addfriend_parameter)
- [プロラインのfreeパラメータ活用方法](https://linelab.jp/prolinefree/)
...
質問や問題があれば、Issueを立てるか、[LINE公式アカウントでお問い合わせください](https://lactewq9.autosns.app/line)。
